        ...
        while not valid_square:
            ...
            try:
                ...
                if val not in game.available_moves():
                    raise ValueError
                valid_square = True # if these are sucessful, then great!
            except ValueError:
...
def GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        max_player = self.letter # youserlf!
        other_player = 'O' if player == 'X' else 'X' # The other players... so whatever letter is

        # first, we want to check if the previous move is a winner
        # this is our base care
        ...
        # initialize some dictionareies
        if player == max_player:
            best = {'position': None, 'score': -math.inf} # each score should maximize (Be larger)
        else:
            best = {'position': None, 'score': math.inf} # eacg score should minimize
        ...
        for possible_moves in state.available_moves():
            ...
            sim_score ['position'] = possible_move # otherwise this will get mess up from the recursion
